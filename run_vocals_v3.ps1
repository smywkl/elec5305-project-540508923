...
if ($Resume) {
    if (-not (Test-Path -LiteralPath $latestCheckpoint)) {
        throw "Cannot resume V3; checkpoint not found: $latestCheckpoint"
    }
    $trainArgs += @("--resume", $latestCheckpoint)
} elseif (Test-Path -LiteralPath $latestCheckpoint) {
    throw "V3 epoch-0 checkpoint already exists. Use -Resume to fine-tune without overwriting it."
}

$displayCommand = '& "{0}" {1}' -f $python, (($trainArgs | ForEach-Object { '"{0}"' -f $_ }) -join " ")
Write-Host "Open-Unmix V3 command:"
Write-Host $displayCommand
Write-Host "Console log: $consoleLog"

if ($DryRun) {
    Write-Host "DryRun: V3 training was not started."
    exit 0
}
...
if ($exitCode -ne 0) {
    throw "V3 trainer exited with code $exitCode. See $consoleLog"
}
exit 0
